...
class PlayerInstance {
    constructor () {
        ...
        this.oBtns=$('player-ui__btn--loop')
    ...
    changeSongState(){
        this.loopMode++;
        this.loopMode=this.loopMode%3;    
        if(this.loopMode === 0){
        this.loopBtn.$el.removeClass('icon-random').addClass('icon-loop')
        }else if(this.loopMode === 1){
        this.loopBtn.$el.removeClass('icon-loop').addClass('icon-single');
        }else if(this.loopMode === 2){
        this.loopBtn.$el.removeClass('icon-single').addClass('icon-random');
        }
    }
    ...
    changeIndexDependLoop (style) { // 根据循环模式切换索引
        ...
        switch ( this.loopMode ) {
            case 0:
                ...
                break;
            case 1:
                this.songIndex = this.songIndex === limit ? result : this.songIndex + 0;
                break;
            case 2:
                this.songIndex = this.songIndex === limit ? result : this.songIndex + Math.floor(Math.random()*4);
                break;
            default: break;
        }
    }
    ...
    bindEvents () { // 绑定事件的方法
        ...
        this.loopBtn = new PlayerBtn('.player-ui__btn--loop',{
            'click': this.changeSongState.bind(this)
        })
        // 歌曲列表 点击换歌
