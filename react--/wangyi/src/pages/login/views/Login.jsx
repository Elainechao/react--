...
import {Logins,Wraps} from './LoginStyle'
import http from 'utils/fetch'
import {getLoginAsync} from './actionCcreator'
...
class Login extends Component {
    constructor(props){
        ...
        // this.getLoginAsync()
        this.state={
            style:{width:"80%",height:".5rem",marginTop:".3rem"},
            style1:{transform:"translateY(8.12rem)"},
            style2:{transform:"translateY(8.12rem)"},
            style3:{color:"#d33a31",backgroundColor:"#fff"},
            style4:{color:"#d33a31",backgroundColor:"#fff"},
            status:'',
            confirmDirty: false,
            autoCompleteResult: [],
            id:''
        ...
        e.preventDefault();
         this.props.form.validateFields( async  (err, values) => {
          ...
          let result = await http.get(
                `/api/login/cellphone?phone=${values.userName}&password=${values.password}`
            )
            
            this.props.fetchData(result.account.id)
            // console.log(this.props.fetchData)
            if(result.code===400 || result.code===415){
                this.setState({
                    status:error
                })
                ...
                // this.props.login=result.account.id
                // this.setState({
                //     id:result.account.id
                // })
                // console.log()
                // this.getLoginAsync(result.account.id)
                // this.fetchData(result.account.id)
                // console.log(this.fetchData())
            }else if(result.code===502 || result.code===509 ){
                this.setState({
                    status:warning
                })
            }
        ...
        return (
            ...
            <div className="login" style={{position:"relative",top:"2rem"}}>
                <div className="login-form" style={{display:"flex",
                    ...
                    <Form  style={{maxWidth: '300px'}} onSubmit={this.handleSubmit} className="login-form">
                        <FormItem>
                        ...
                        <FormItem>
                            ...
                            <div className="login-form-forgot" style={{float: 'right'}}>忘记密码</div>
            ...
            <div className="place" style={this.state.style2} 
            onClick={()=>{
               this.setState({
                   style2:{transform:"translateY(8.12rem)"},
                   style4:{color:"#d33a31",backgroundColor:"#fff"}
