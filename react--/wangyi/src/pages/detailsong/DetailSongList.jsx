...
import {getSimisongsAsync,getSimiplaylistAsync,getSongdetailAsync,getMusicurlAsync,getSonglistAsync,getCommentmusicAsync} from './actioncreator'
import {withRouter} from 'react-router-dom'
import { DetailsMusicContainer,WrapImg } from './DetailsStyled'
import MusicBook from 'components/musicbook/MusicBook'
import Comment from 'components/comment/Comment'
import BSrcoll from 'better-scroll'
const mapState=state=>{
    return({
        simiplaylist:state.detail.simiplaylist,
        musicurl:state.detail.musicurl,
        songlist:state.detail.songlist,
        songdetail:state.detail.songdetail,
        commentmusic:state.detail.commentmusic,
        simisongs:state.detail.simisongs,
    })
}
const mapDispatch =dispatch =>({
    fetchData(){
        let id=this.location.pathname.split('/')[2]
        dispatch(getSimisongsAsync(id))
        dispatch(getSimiplaylistAsync(id))
        dispatch(getMusicurlAsync(id))
        dispatch(getSongdetailAsync(id))
        dispatch(getCommentmusicAsync(id))
        dispatch(getSonglistAsync(id))
    }
})
class DetailSongList extends Component {
    constructor(props){
        super(props)
        this.fetchData()
        this.state={
            ...
            title:"playlyric",
            currWordIndex:0,
            style2:{transform: "translateY(-78px)"}
        }
    ...
    componentWillReceiveProps(nextprops){
        ...
        if(this.props.songlist !== nextprops.songlist){
            if(this.props.songlist.version){
                let songlyric =this.props.songlist.lyric
                let lyrics = songlyric.split("\n");
            let lrcObj = {};
            ...
            setInterval(()=>{
                ...
                for (var j = 0;j <= str.length;j++) {
                    top += 32
                    // console.log(a > Number(str[j+1]) )

                    // console.log(top)
                    if(a > Number(str[j+1]) && a < Number(str[j]) ){

                        that.setState({
                            style2:{transform:`translateY(-${top}px)`}
                        })
                    }
                    // break
                }
                                        ...
                                        <div>
                                            ...
                                            <div className="m-song-wrap">
                                                <div className="m-song-disc">
                                                    ...
                                                    <span className="m-song-plybtn " style={this.state.style1} onClick={()=>this.handlerplayClick()}></span>
                                                </div>
                                                <div className="m-song-clickarea" onClick={()=>this.handlerpauseClick()}>
                                                {  
                                                        musicurl.map(music=>(
                                                        <audio controls="" autoplay="true" className="media" height="80px" loop preload="auto" key={music.id}>
                                            ...
                                            {
                                                songdetail.map(item=>(
                                                    <h2 className="m-song-h2"  key={item.id}>
                                                    <span className="m-song-sname">{item.name}</span>
                                                    <span className="m-song-gap">-</span>
                                                    <b className="m-song-autr">{item.ar[0].name}</b>
                                                </h2>
                                                ))
                                            }   
                                                <div className="m-song-lrc f-pr">
                                                    <div className="m-song-scroll">
                                                        <div className="m-song-iner" id={this.state.title} style={this.state.style2}>
                                                        {
                                                            songlyric.split("\n").map((song,index)=>(
                                                            <p key={index} className="m-song-lritem j-lritem">{song.split("]").pop()}</p>
                                                            ))
                                                        } 
                                                        </div>
                                                    </div>
                                            ...
                                            <div className="m-link link-enter-done">查看完整歌词 ></div>
                                            <div className="m-giude arowopy-enter-done" onClick={()=>{this.handlerLogoClick()}}>
                                                <i className="arr ani"></i>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="comment-list">
                                        <MusicBook {...simiplaylist}  {...simisongs}></MusicBook>
                                        <Comment {...commentmusic} title={this.state.change}></Comment>
                                    </div>
                                </div>
                            </div>
                            <div className="playsrc" onClick={()=>{
                                this.props.history.push('/home')} }>
                                    <img src="https://timgsa.baidu.com/timg?image&quality=80&size=b9999_10000&sec=1555656314&di=43a1a5775193aa55403d961d3b320846&imgtype=jpg&er=1&src=http%3A%2F%2Fimg.ph.126.net%2FNufMgqQbR9PBA3rZvJVaeg%3D%3D%2F2312598408655325861.png" alt=""/>
                                    GO  HOME
                             </div>
                    </div>
                    </div>
    ...
    componentDidMount(){
        this.fetchData()
       this.bsrcoll=new BSrcoll('.m-song_nor',{click:true})
    }
    handlerLogoClick(){
        this.bsrcoll.scrollTo(0,-752)
    }
    handlerpauseClick(){
        let video = document.getElementsByClassName("media")
        video[0].pause()
        this.setState({
...
export default connect(mapState,mapDispatch)(withRouter(DetailSongList))
